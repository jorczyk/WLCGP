...
# frames_paths = iter(sorted(glob.glob(args.stream_path + "*.jpg")))
frames = os.listdir(args.stream_path)
# frames = (glob.glob("C:\\Users\\Piotr\\Desktop\\SEM X\\Magisterka\\KOD\\ChokePointDataset\\P1E_S1_C1\\25"))
# frames = next(os.walk(args.stream_path))[2]
# path = '00004345.jpg'
# current_frame = cv2.imread(path)
# cv2.imshow("DUPA",current_frame)
# cv2.waitKey(0)
# cv2.destroyAllWindows()

for frame in frames:
	# print frame
	path = args.stream_path + '/' + frame
	current_frame = cv2.imread(path)
	# im = cv2.cv.LoadImage(path)
...
# 	# current_frame = cv2.imread(next(frames_paths))
# 	# gray_frame = cv2.cvtColor(current_frame, cv2.COLOR_BGR2GRAY)
# except IOError:
